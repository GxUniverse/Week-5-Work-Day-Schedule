...
// current hour in 24-hour time?
  for (var i = 9; i <= 17; i++) {
    (function (index) {
      ...
      var textBlock = timeBlock.find('.description');
...
// attribute of each time-block be used to do this?
      var schedule = localStorage.getItem('hour-' + index);
      if (schedule) {
        textBlock.val(schedule);
      }

      saveButton.on('click', function () {
        localStorage.setItem('hour-' + index, textBlock.val());
      });
    })(i);
  }
});
